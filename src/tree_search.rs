...
impl <T: SearchableState> SearchNode<T> {
    ...
    pub fn most_visited_child(&self) -> NodeRef<T> {
        self.children.iter().max_by_key(|&c| c.borrow().visits ).expect("most_visited_child() called on terminal node").clone()
    }
    ...
    pub fn select_most_promising_child(&mut self) -> NodeRef<T> {
        ...
        self.children.first().expect("SearchNode::select_most_promising_child failed: no children").clone()
    }
...
fn expand_node_by_move<T: SearchableState>(node_ref: NodeRef<T>, move_idx: usize, ctx: &mut T::C) -> NodeRef<T> {
    ...
    let new_node = SearchNode {
        ...
        player_just_moved: node.state.active_player().expect("State with move must have active player"),
...
pub fn find_best_move<T: SearchableState>(root_state: T, max_iters: i32, ctx: &mut T::C, debug: bool) -> T::M {
    ...
    let just_moved: T::P = root_state.all_players().last().cloned().expect("Players must not be empty");
