...
impl tree_search::SearchableState for Game {
    ...
    fn game_result(&self) -> Option<tree_search::Winners<Self::P>> {
        if self.is_game_over() {
            ...
            let winners = scores.iter().filter_map(|&(pid, score)| {
                if score > 0.0 {
                    Some(pid)
                } else {
                    None
    ...
    fn all_moves(&self) -> Vec<Self::M> {
        ...
        let d = self.pending_decision.as_ref().expect("Game::all_moves called without pending decision");
...
impl Decider for SearchDecider {
    fn description(&self) -> String { return "Tactician".into(); }
        
    fn make_decision(&mut self, g: &Game) -> Vec<CardIdentifier> {
        {
            let d = g.pending_decision.as_ref().expect("SearchDecider::make_decision called without pending decision");
            if let Some(choice) = hard_coded_decision(&d) {
                return choice;
            }
        }
